...
import { 
  Lock, User, Key, ArrowRight, X, ShieldAlert, 
  GraduationCap, Building2, Layers, Sparkles, FileSpreadsheet 
} from 'lucide-react';
    ...
    <>
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-6 bg-obsidian-950/95 backdrop-blur-2xl animate-fade-in">
        <div className="bg-obsidian-900 border border-gold-500/40 rounded-xl max-w-md w-full p-5 sm:p-8 relative shadow-2xl space-y-5 sm:space-y-6 max-h-[92vh] overflow-y-auto">
          ...
          {portalType === 'staff' && (
            <div className="p-3.5 rounded-lg bg-gold-500/10 border border-gold-500/30 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono text-gold-300 uppercase font-bold flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-gold-400" />
                  <span>Admissions Submissions Suite</span>
                </span>
                <span className="text-[9.5px] px-2 py-0.5 rounded bg-gold-500 text-obsidian-950 font-mono font-extrabold uppercase">
                  Active
                </span>
              </div>
              <p className="text-[11px] text-neutral-300 font-light leading-relaxed">
                Review incoming student applications, tour bookings, export CSV, or configure Google Sheets & Email sync.
              </p>
              <button
                type="button"
                onClick={() => setShowAdminDashboard(true)}
                className="w-full py-2 rounded bg-gold-500 hover:bg-gold-400 text-obsidian-950 font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-md shadow-gold-500/20"
              >
                <Layers className="w-3.5 h-3.5" />
                <span>Launch Admin Submissions Console</span>
              </button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  required
                  placeholder={portalType === 'student' ? 'e.g. AFK-2026-0412' : 'e.g. admin@afrakfashionschool.com'}
                  value={studentId}
            ...
            <div className="flex items-center justify-between text-[11px] text-neutral-400">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input type="checkbox" defaultChecked className="rounded bg-obsidian-950 border-white/20 text-gold-500" />
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  <span>{portalType === 'staff' ? 'Open Staff Console' : 'Sign In To Portal'}</span>
                </>
              )}
            </button>
          </form>
      ...
      {/* Full Admin Submissions & Intelligence Console */}
      <AdminDashboardModal
